...
$arAllOptions = array(
	array("step_time", GetMessage("FILEINSPECTOR_STEP_TIME"), "15", array("text", 5)),
);
$aTabs = array(
	array(
		"DIV" => "edit1",
		"TAB" => GetMessage("FILEINSPECTOR_TAB_SET"),
		"ICON" => "af_settings",
		"TITLE" => GetMessage("FILEINSPECTOR_TAB_TITLE_SET")),
);
...
if($REQUEST_METHOD=="POST" && strlen($Update.$Apply.$RestoreDefaults)>0 && check_bitrix_sessid())
{
	...
	{
		COption::RemoveOption("alexkova.fileinspector");
	}
	else
	{
		foreach($arAllOptions as $arOption)
		{
			...
			if($arOption[2][0]=="checkbox" && $val!="Y")
				$val="N";
			COption::SetOptionString("alexkova.fileinspector", $name, $val, $arOption[1]);
		}
	}
...
<?$tabControl->BeginNextTab();?>
	<?
	foreach($arAllOptions as $arOption):
		$val = COption::GetOptionString("alexkova.fileinspector", $arOption[0], $arOption[2]);
		$type = $arOption[3];
	?>
	<tr>
